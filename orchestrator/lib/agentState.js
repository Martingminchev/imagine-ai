...
/**
 * Load or create the global AgentState for a user.
 *
 * The agent has a single persistent identity per user — one set of core
 * values, character traits, mood, selfModel, userModel, drives, hum, and
 * circulation. Conversations (projects) are workspaces; the agent brings
 * its full self to each one.
 *
 * If no state exists, creates one using a personality preset and injects
 * seed memories (backdated 7 days so they appear as founding memories).
 *
 * @param {string} userId - The user identifier (defaults to 'anonymous').
 * @param {string|null} personalityId - Personality preset id (defaults to 'architect').
 * @returns {Promise<Object>} The AgentState document (existing or newly created).
 */
async function loadOrCreateState(userId = 'anonymous', personalityId = null) {
  let state = await AgentState.findOne({ userId })

  if (!state) {
    const preset = getPersonality(personalityId || 'architect')
    const createData = {
      userId,
      personality: preset?.id || 'architect'
    }
    ...
    console.log(`  → AgentState: born as "${preset?.name || 'Architect'}" (first interaction)`)

    // Inject seed memories — backdated 7 days so they appear as distant founding memories
 ...
 *
 * The manifesto is the "who I really am" injection — it translates
 * numeric state values into natural-language descriptions that shape
 * the AI's tone, word choices, and behavioral tendencies.
 *
 * Sections:
 *   - CORE VALUES: Fundamental nature (honesty, curiosity, empathy, etc.)
 *   - CHARACTER: Behavioral style (directness, warmth, humor, etc.)
 *   - CURRENT STATE: Dynamic mood, energy, trust, frustration, etc.
 *   - SELF: Identity, beliefs, open questions from selfModel
 *   - ABOUT THE USER: Relationship summary, preferences, communication style
 *
 * @param {Object} state - The AgentState document (or plain object).
 * @returns {string} The formatted manifesto block.
 */
...
/**
 * Build the state-update prompt for LLM reflection.
 *
 * After each exchange the AI reflects on what happened and updates
 * its internal state. This helper produces the system + user prompts
 * that drive that reflection.
 *
 * Update rules baked into the prompt:
 *   - core values:  MAX +/-0.03 per turn, only on truly transformative events
 *   - character:    MAX +/-0.05 per turn, slow personality drift
 *   - dynamic:      free update each turn (mood, energy, trust, etc.)
 *   - userModel:    update as you learn about the person
 *   - selfModel:    update when you learn something about yourself
 *
 * @param {Object} currentState - The current AgentState document.
 * @param {string} userMessage  - The user's message in this exchange.
 * @param {string} aiResponse   - The AI's reply in this exchange.
 * @param {number} dissonance   - Dissonance score (0.0 = familiar, 1.0 = new territory).
 * @param {number} turnCount    - Current turn count before this update.
...
/**
 * Run the state update after an exchange.
 *
 * Calls the LLM with the reflection prompt, parses the JSON response,
 * applies safety clamps to every field, tracks stuck concerns, logs
 * major shifts, increments turnCount, and persists.
 *
 * Safety clamps:
 *   - All numeric core/character/dynamic values are clamped to [0, 1]
 *   - String fields are length-capped (mood: 100, currentConcern: 1000, etc.)
 *   - Array fields are element-capped and string-length-capped
 *   - dominantStyle is capped at 50 chars
 *   - majorShiftLog is capped at 50 entries (keeps most recent)
 *
 * @param {Object} state       - The AgentState document to update.
 * @param {string} userMessage - The user's message.
 * @param {string} aiResponse  - The AI's response.
 * @param {number} dissonance  - Dissonance score for this exchange.
 * @param {string} model       - LLM model identifier.
 * @param {Object} apiKeys     - API key configuration for generate().
 * @returns {Promise<Object>} The updated (and saved) AgentState document.
 */
async function updateAgentState(state, userMessage, aiResponse, dissonance, model, apiKeys) {
  ...
  try {
    ...
    // ── Core: clamp all numeric values to [0, 1] ─────────────────────
    if (updated.core) {
    ...
    // ── Character: clamp numerics, allow string for dominantStyle ─────
    ...
    // ── Dynamic: free update each turn ────────────────────────────────
    if (updated.dynamic) {
      if (typeof updated.dynamic.mood === 'string') {
        state.dynamic.mood = updated.dynamic.mood.slice(0, 100)
      }
 ...
 *
 * A concern is considered "stuck" when it has persisted unchanged for
 * 3 or more consecutive turns. This triggers archival / contemplation
 * flows upstream.
 *
 * @param {Object} state - The AgentState document.
